...
​	在此上下文中，`MsgA`是从客户端发送的Protobuf消息，`MsgB`是从服务端返回的Protobuf消息。

### Server-streaming methods 服务端流式方法
...
服务端端处理程序可以通过该参数的`Send`方法向客户端发送一系列的Protobuf消息。服务端到客户端的流的结束由处理程序方法的`return`语句引发。
...
​	服务端端处理程序可以重复调用该参数上的`Recv`方法，以接收来自客户端的完整消息流。一旦达到流的末尾，`Recv`将返回`(nil, io.EOF)`。通过在`<ServiceName>_FooServer`参数上调用`SendAndClose`方法，服务端发送单个响应消息。请注意，`SendAndClose`必须且只能被调用一次。
...
​	服务端端处理程序可以重复调用该参数上的`Recv`方法，以读取客户端到服务端的消息流。一旦达到客户端到服务端流的末尾，`Recv`将返回`(nil, io.EOF)`。通过重复调用`<ServiceName>_FooServer`参数上的`Send`方法，发送服务端到客户端的响应消息流。对于服务端到客户端的流，流的结束由双向方法处理程序的`return`语句指示。
...
在这个上下文中，`MsgA`是客户端发送到服务端的单个请求，`MsgB`包含服务端发送回的响应。
...
当客户端在存根上调用`Foo`方法时，流式开始。然后，客户端可以重复调用返回的`<ServiceName>_FooClient` *stream* 上的`Recv`方法，以读取服务端到客户端的响应流。一旦完全读取了服务端到客户端的流，`Recv`方法将返回`(nil, io.EOF)`。
...
当客户端在存根上调用`Foo`方法时，流式开始。然后，客户端可以重复调用返回的`<ServiceName>_FooClient`流上的`Send`方法，以发送客户端到服务端的消息流。必须仅调用一次`CloseAndRecv`方法来关闭客户端到服务端的流，并从服务端接收单个响应消息。
...
当客户端在存根上调用`Foo`方法时，流式开始。然后，客户端可以重复调用返回的`<SericeName>_FooClient`流上的`Send`方法，以发送客户端到服务端的消息流。客户端还可以重复调用此流上的`Recv`方法，以接收完整的服务端到客户端的消息流。
...
对于服务端到客户端的流，通过流的`Recv`方法返回值为`(nil, io.EOF)`来表示流结束。对于客户端到服务端的流，客户端可以通过在流上调用`CloseSend`方法来表示流结束。
